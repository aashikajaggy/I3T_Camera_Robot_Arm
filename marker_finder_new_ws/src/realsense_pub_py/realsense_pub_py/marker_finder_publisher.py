...
class  marker_finder_publisher(Node):
    def __init__(self):
        super().__init__('marker_finder_publisher')
        self.publisher_ = self.create_publisher(Float32MultiArray, 'marker_location', 10)
        time_period = 0.5
    ...
    def timer_callback(self):
        ...
        self.get_logger().info(f'Publishing: {vector}')
        self.i += 1
...
def main(args=None):
    ...
    marker_finder_publisher1 = marker_finder_publisher()

    rclpy.spin(marker_finder_publisher1)
    marker_finder_publisher1.destroy_node()
    rclpy.shutdown()
